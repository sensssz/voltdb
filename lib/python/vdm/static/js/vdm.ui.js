...
(function (window) {
    var iVdmUi = (function () {
        ...
        this.displayDeployment = function(deployment){
            ...
            {
                clusterObjects.chkSecurity.iCheck('check')
            }
            else
            {
                clusterObjects.chkSecurity.iCheck('uncheck')
            }
        ...
        this.displayExportProperties = function(){
            ...
            for(var i = 0; i < VdmUI.exportPropertyList.length; i++){
                ...
                exportList += '<tr data-property=\''+ JSON.stringify(VdmUI.exportPropertyList[i]) +'\' class="child-row-4 subLabelRow parentprop exportPropertyClass" id="row-41-' + i + ' " style="display:'+ showHide +'">' +
                       ...
                       exportList += '<td><a id="btnUpdateExport_'+ i +'" class="btnUpdateExport" href="javascript:void(0);"data-toggle="modal" data-target="#plusExport"><span class="editIcon"></span></a></td>' +
                                    '</tr>';
            ...
            if(exportList == ''){
                exportList = '<tr class="child-row-4 subLabelRow exportPropertyClass" id="trExport"  style="display:'+ showHide +'">' +
        ...
        this.displayImportProperties = function(){
            ...
            for(var i = 0; i < VdmUI.importPropertyList.length; i++){
                ...
                importList += '<tr data-property=\''+ JSON.stringify(VdmUI.importPropertyList[i]) +'\' class="child-row-7 subLabelRow importParentProp  importPropertyClass" id="row-71-' + i + ' " style="display:'+ showHide +'">' +
                       ...
                       importList += '<td><a class="btnUpdateImport" href="javascript:void(0);"data-toggle="modal" data-target="#plusImport"><span class="editIcon"></span></a></td>' +
                                    '</tr>';
                if(property.length > 0){
                    for(var j = 0; j < property.length; j++){
                       var propertyName = property[j].name;
            ...
            if(importList == ''){
                importList = '<tr class="child-row-7 subLabelRow importPropertyClass" id="trExport"  style="display:'+ showHide +'">' +
                    '<td class="configLabel">No configuration available.</td>' +
                    '<td>&nbsp;</td>' +
                    '<td>&nbsp;</td>' +
                    '<td>&nbsp;</td>' +
        ...
        this.displaySecurity = function(){
            ...
            for(var i = 0; i < VdmUI.userSecurityList.length; i++){
                ...
                var roles = VdmUI.userSecurityList[i].roles;
                    ...
                    '<a class="btnUpdateSecurity" href="javascript:void(0);" data-toggle="modal" data-target="#plusSecurity"><span class="editIcon"></span></a><td></tr>'
                }
            $('.securityList').remove();
            if(securityList == ''){
